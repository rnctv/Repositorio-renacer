...
<div class="row g-2 align-items-end flex-wrap fin-filters">
  <div class="col-auto">
    ...
    <input type="month" id="filter-period" class="form-control" value="<?php echo e($currentPeriod ?? now()->format('Y-m')); ?>">
  </div>
  <div class="col-auto">
  ...
  <div class="col-auto d-none">
    ...
    <select id="filter-subcategory" class="form-select">
      ...
      <?php $__currentLoopData = ($subcategories ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <option value="<?php echo e($s->id); ?>" data-category="<?php echo e($s->category_id); ?>"><?php echo e($s->name); ?></option>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </select>
  </div>
  <div class="col-auto">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const editUrlBase    = <?php echo json_encode(route('finanzas.index'), 15, 512) ?>;
  const destroyUrlBase = <?php echo json_encode(url('finanzas'), 15, 512) ?>;
  const dataUrl        = <?php echo json_encode(route('finanzas.historial.data'), 15, 512) ?>;
  const csrfToken      = <?php echo json_encode(csrf_token(), 15, 512) ?>;
  const currentPeriod  = <?php echo json_encode($currentPeriod ?? now()->format('Y-m'), 15, 512) ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/laravel/resources/views/finanzas/historial.blade.php ENDPATH**/ ?>
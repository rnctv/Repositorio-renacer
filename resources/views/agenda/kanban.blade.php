...
{{-- Badges del carrusel (CSS externo) --}}
<link rel="stylesheet" href="{{ asset('css/kanban-badges.css') }}?v={{ $assetManifest['css/kanban-badges.css'] ?? '' }}">
@endpush
...
<div id="kanbanBoard"
     ...
     {{-- IMPORTANTÍSIMO: Defaults desde config/app.php o .env (Longaví) --}}
...
{{-- EXPONE defaults al JS como respaldo para evitar cualquier hardcode --}}
<script>
  (function () {
...
@push('scripts')
  <script>
    window.GMAPS_KEY = @json($gmapsKey);
    window.DEFAULT_RECIPIENTS = @json($recipients ?? []);
  </script>

  <script src="{{ asset('js/kanban.js') }}?v={{ $assetManifest['js/kanban.js'] ?? '' }}"></script>
  <script src="{{ asset('js/kanban.openSend.polyfill.js') }}?v={{ $assetManifest['js/kanban.openSend.polyfill.js'] ?? '' }}"></script>   {{-- <— NUEVO --}}
  <script src="{{ asset('js/kanban.override.js') }}?v={{ $assetManifest['js/kanban.override.js'] ?? '' }}"></script>
  <script src="{{ asset('js/kanban-badges.js') }}?v={{ $assetManifest['js/kanban-badges.js'] ?? '' }}" defer></script>
@endpush

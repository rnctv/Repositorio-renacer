...
@php
  // Fallbacks por si el Controller no los envía
  $gmapsKey = $gmapsKey ?? (config('services.google.maps_key') ?? env('GOOGLE_MAPS_KEY',''));
...
<style>
  ...
  .day-pill{min-width:64px;border:1px solid var(--border);background:#fff;border-radius:12px;padding:.35rem .5rem;display:flex;flex-direction:column;align-items:center;gap:.2rem;cursor:pointer}
...
<div id="kanbanBoard"
     data-endpoint-list="{{ route('agenda.list') }}"
     data-endpoint-move="{{ url('/agenda') }}"
     data-endpoint-counts="{{ route('agenda.counts') }}"
     data-endpoint-pendientes="{{ route('agenda.pendientes') }}"
      ...
      <form id="taskForm">
        <div class="g">
          ...
          <div>
            ...
            <input class="inp" name="tipo" value="instalacion" required />
          </div>
          <div>
            <label class="lbl">Plan</label>
            <input class="inp" name="plan" value="INTERNET 600" />
          </div>
          <div>
            <label class="lbl">Título (opcional)</label>
            <input class="inp" name="titulo" placeholder="Se autogenera si lo dejas vácio" />
          </div>
          ...
          {{-- Campos ocultos para cliente seleccionado / coords temporales --}}
...
<div class="mask" id="viewMask" aria-hidden="true">
  <div class="dialog" role="dialog" aria-modal="true" aria-labelledby="viewTitle">
    <div class="dlg-hd">
      <div class="dlg-ttl" id="viewTitle">Detalle de la actividad</div>
      <button class="dlg-x" type="button" data-close-view>✕</button>
    </div>
    <div class="dlg-bd">
      <div class="g">
        <div><label class="lbl">Nombre</label><input class="inp" id="v_cliente"></div>
        <div><label class="lbl">Teléfono</label><input class="inp" id="v_telefono"></div>
        <div class="g-1"><label class="lbl">Dirección</label><input class="inp" id="v_direccion"></div>
        <div><label class="lbl">Tipo</label><input class="inp" id="v_tipo"></div>
        <div><label class="lbl">Plan</label><input class="inp" id="v_plan"></div>
        <div><label class="lbl">Creada</label><input class="inp" id="v_fecha"></div>
        <div><label class="lbl">Días transcurridos</label><input class="inp" id="v_estado"></div>
        <div class="g-1"><label class="lbl">Descripción</label><textarea class="txt" id="v_desc"></textarea></div>
      </div>
  ...
  <div class="dialog" role="dialog" aria-modal="true" aria-labelledby="mapTitle">
    ...
    <div class="dlg-bd">
      ...
      <div class="actions" style="margin-top:.75rem">
        ...
        <button class="btn primary" type="button" id="saveMap">Guardar ubicación</button>
      </div>
    </div>
  </div>
</div>
...
@push('scripts')
  <script src="{{ asset('js/kanban.js') }}?v=3.52"></script>
@endpush
